...
@gin.configurable
class CriticVCQL(nn.Module):
    def __init__(self, dim_observation, dim_action):
        super(CriticVCQL, self).__init__()
...
@gin.configurable
class ActorVCQL(nn.Module):
    def __init__(self, dim_observation, dim_action):
        super(ActorVCQL, self).__init__()
...
class VCQL:
    def __init__(self, dim_obs, dim_actions, gamma, tau, critic_lr, actor_lr, buffer_size, sample_size, network_random_seed=1,
                 explore_noise=0.1, Q_train_ite=2, Actor_train_ite=1, cql_rate=0.0002, vcql_rate=1,
                 cql_num_sample=10, vcql_num_sample=100, ACTION_MAX=10, ACTION_MIN=0, TD_error_rate=0.5,
                 TD_switch="on", CQL_switch="off", VCQL_switch="off", explore_size=10000):
        self.num_of_states = dim_obs
        self.num_of_actions = dim_actions
        self.critic_lr = critic_lr
        self.actor_lr = actor_lr
        self.old_Q = None
        print("INFO: There is no old Q network yet")
        self.explore_noise = explore_noise
        self.Q_train_ite = Q_train_ite
        self.Actor_train_ite = Actor_train_ite
        self.cql_rate = cql_rate
        self.vcql_rate = vcql_rate
        self.TD_error_rate = TD_error_rate
        self.cql_num_sample = cql_num_sample
        self.vcql_num_sample = vcql_num_sample
        ...
        self.VCQL_switch = VCQL_switch

        print("TD loss switch: " + self.TD_switch)
        print("CQL loss switch: " + self.CQL_switch)
        print("VCQL loss switch: " + self.VCQL_switch)

        self.Q_png_save_path = save_Q_png_file_path(self.TD_error_rate, self.cql_rate, self.vcql_rate, 
                                                    self.TD_switch, self.CQL_switch, self.VCQL_switch, self.explore_size) + "_random_seed_" + str(self.network_random_seed) + ".png" 
        ...
        self.actors = ActorVCQL(self.num_of_states, self.num_of_actions)
        self.critics = CriticVCQL(self.num_of_states, self.num_of_actions)
    ...
    def load_old_Q_network(self, load_path="saved_models/sorl/iteration_0/critic.pkl"):
        self.old_Q = torch.load(load_path, map_location=torch.device('cpu'))
        # cuda usage
        if self.use_cuda:
            self.old_Q.cuda()
        print("INFO: successfully load the Q network")
    ...
    def train(self):
        ...
        if self.if_full:
            ...
            for i in range(self.Q_train_ite):
                ...
                # V-CQL terms KL divergence between updating Q network and old Q network
                if self.VCQL_switch == "on":
                    print("VCQL switch on")
                    vcql_states = torch.stack([states for i in range(self.vcql_num_sample)]).reshape(
                        self.vcql_num_sample * self.sample_size,
                        self.num_of_states).type(self.FloatTensor)
                    vcql_sample_actions = torch.linspace(self.ACTION_MIN / 5 - 1, self.ACTION_MAX / 5 - 1, self.vcql_num_sample).type(self.FloatTensor)
                    vcql_sample_actions = torch.stack([vcql_sample_actions for i in range(self.sample_size)]).permute(1,
                                                                                                                    0).reshape(
                        self.vcql_num_sample * self.sample_size, 1).type(self.FloatTensor)

                    new_Q_distribute = self.critics(vcql_states, vcql_sample_actions).reshape(self.vcql_num_sample,
                                                                                            self.sample_size).permute(1,
                                                                                                                        0)
                    old_Q_distribute = self.old_Q(vcql_states, vcql_sample_actions).reshape(self.vcql_num_sample,
                                                                                            self.sample_size).permute(1, 0)
                    loss_vcql = F.kl_div(new_Q_distribute.softmax(dim=-1).log(), old_Q_distribute.softmax(dim=-1),
                                        reduction='sum')

                    # direct MSE loss
                    # loss_vcql = torch.nn.MSELoss()(new_Q_distribute, old_Q_distribute.detach())

                    loss_Q += self.vcql_rate * loss_vcql
